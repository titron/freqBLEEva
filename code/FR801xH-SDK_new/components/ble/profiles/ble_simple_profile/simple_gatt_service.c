...
#define TYCON_DEBUG
#include "Sht3x.h"
bool sp_svc_ntf_flag = false;
gatt_ntf_t ntf_atttemp;
/*********************************************************************
 * @fn      sp_gatt_write_cb
 *
...
static void sp_gatt_write_cb(uint8_t *write_buf, uint16_t len, uint16_t att_idx,uint8_t conn_idx)
{
	for (int i = 0; i < len; i++)
	{
		...
		{	
			...
			ntf_atttemp.att_idx = SP_IDX_CHAR4_VALUE;
			ntf_atttemp.conidx = conn_idx;
			ntf_atttemp.svc_id = sp_svc_id;
			ntf_atttemp.data_len = ble_ntf_length;
			ntf_atttemp.p_data = ble_ntf_buff;
			gatt_notification(ntf_atttemp);
			
//			co_printf("Received command from Mobile\r\n");
            #else
			ntf_atttemp.att_idx = SP_IDX_CHAR4_VALUE;
			ntf_atttemp.conidx = conn_idx;
			ntf_atttemp.svc_id = sp_svc_id;
			ntf_atttemp.data_len = 6;
			uint8_t ntf_data[]= "ACK OK";
			ntf_atttemp.p_data = ntf_data;
			gatt_notification(ntf_atttemp);
		#endif
		}
#endif
	...
	if (uuid == GATT_CLIENT_CHAR_CFG_UUID)
	{
		...
		if (att_idx == SP_IDX_CHAR4_CFG)
		{
			...
			if((sp_char4_ccc[0] == 1) && (sp_char4_ccc[1] == 0))
			{
				...
				ntf_atttemp.p_data = ntf_data;
				gatt_notification(ntf_atttemp);
				co_printf("Notification Data: 0x%x \r\n", ntf_data);
			}
			else
			{
